...
profile.HandleMidcast = function()
    ...
    if (action.Skill=="Elemental Magic") then
        if (sets.WHM[MagStyle][action.Element]) then
            equip(sets.WHM[MagStyle][action.Element]) 
        else
             equip(sets.WHM[MagStyle])
        end
    end
    ...
    if (action.Name=="Impact") then
        equip(sets.WHM['Magic']['Impact'])
    end
    
    weatherCheck(action.Element, action.Skill) -- Calling skill is checked by weatherCheck. This is called by common-rules, but needs to be reset after checking WHM specific ele sets
end
